...
\hlkwd{canvas_collatz}\hldef{(}
  \hlkwc{colors}\hldef{=}\hlkwd{colorPalette}\hldef{(}\hlsng{"vrolik4"}\hldef{),}
  \hlkwc{background} \hldef{=} \hlsng{"#08bfdf"}\hldef{,}
  \hlkwc{n} \hldef{=} \hlnum{300}\hldef{,}
  \hlkwc{angle.even} \hldef{=} \hlnum{0.0193}\hldef{,}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{c}\hlkwb{=}\hlkwd{which}\hldef{(y}\hlopt{==}\hlnum{40}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\textbf{Solution: This approach only works when the solution for x is a whole number.}
\end{enumerate}
\end{enumerate}
